...
class HomeView(AdminIndexView):
    ...
    def index(self, session=None):
        ...
        # 1、filter_owner 拦截的用户
        # 2、filter_date 拦截的日期
        # 3、filter_date 拦截的状态 success/failed
        ...
        if arg_filter_date :
            arg_filter_date = dt.datetime.strptime(arg_filter_date, '%Y-%m-%d')
            arg_filter_date = timezone.convert_to_utc(arg_filter_date)

        list_dag_id = []
        # 当存在过滤条件时候的判断,将符合条件的信息放入列表中,在后面查询时候进行筛选使用
        ...
        if do_filter and owner_mode == 'ldapgroup':
            ...
            sql_query = sql_query.filter(
                ...
                DM.dag_id.in_(list_dag_id) if len(list_dag_id) > 0 else DM.dag_id == ""
            )
        else:
            sql_query = sql_query.filter(
                ~DM.is_subdag, DM.is_active
        ...
        if do_filter and owner_mode == 'ldapgroup':
            ...
            webserver_dags = {
                dag.dag_id: dag
                for dag in unfiltered_webserver_dags
                if (dag.owner == arg_filter_owner_query and len(list_dag_id) > 0 and dag.dag_id in list_dag_id)
                }
        else:
            webserver_dags = {
                dag.dag_id: dag
